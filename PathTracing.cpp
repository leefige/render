...
Color PathTracing::tracing(Light light, int depth, vector<Object *> &objs) {
    ...
    double maxC = col.getMaxChannel();
    depth++;
    Color e = obj->getEmit();

    if (depth > 5) {
        if (rand() / (double)RAND_MAX * MAX_COLOR < maxC) {
            col = col * (MAX_COLOR / maxC);
        }
        else {
            return e;
        }
    ...
    ReflectType ref_tp = obj->getRefType();
    if (ref_tp == SPECULAR)
    {
        return e + col.mix(tracing(Light(pos, light.direct - light.direct * n * n * 2), depth, objs));
    }
    ...
    else if (ref_tp == DIFFUSE)
    {
        ...
        return e + col.mix(tracing(Light(pos, d), depth, objs));
    }

    else if (ref_tp == REFRACTIVE)
    {
        ...
        if (cos2t < 0) {
            return e + col.mix(tracing(reflRay, depth, objs));
        }
        ...
        if (depth > 2) {
            if (rand() / (double)RAND_MAX < p) {
                return e + col.mix(tracing(reflRay, depth, objs) * rp);
            }
            return e + col.mix(tracing(Light(pos, tdir), depth, objs) * tp);
        }
        return e + col.mix(tracing(reflRay, depth, objs) * re + tracing(Light(pos, tdir), depth, objs) * tr);
    }
}
...
#pragma omp parallel for       // OpenMP
    for (int y = 0; y < h; y++)
        for (int x = 0; x < w; x++) {
            printf("\rProcess %5.2lf%%", (y * w + x) * 100 / double(h * w));
            for (int sy = 0; sy < 2; sy++)
                for (int sx = 0; sx < 2; sx++) {
                    ...
                    film.c[(h - y - 1) * w + x] = film.c[(h - y - 1) * w + x] + pix.normalize() * 0.25;
                }
        }
}